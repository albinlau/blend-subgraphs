...
function saveUserReserveAHistory(
  ...
  let bTokenBalanceHistoryItem = new BTokenBalanceHistoryItem(
...
function tokenBurn(
  ...
  let bToken = getOrInitSubToken(event.address);
  let userReserve = getOrInitUserReserve(from, bToken.underlyingAssetAddress, event);
  let poolReserve = getOrInitReserve(bToken.underlyingAssetAddress, event);
  ...
  poolReserve.totalBTokenSupply = poolReserve.totalBTokenSupply.minus(userBalanceChange);
...
function tokenMint(
  ...
  poolReserve.totalBTokenSupply = poolReserve.totalBTokenSupply.plus(userBalanceChange);
  ...
  if (pool && pool.pool) {
    ...
    const reserveData = poolContract.try_getReserveData(
      Address.fromString(bToken.underlyingAssetAddress.toHexString())
    );
    if (!reserveData.reverted) {
      ...
      log.error('error reading reserveData. Pool: {}, Underlying: {}', [
        ...
        bToken.underlyingAssetAddress.toHexString(),
      ]);
    }
  }
  ...
  if (
    ...
    let userReserve = getOrInitUserReserve(onBehalf, bToken.underlyingAssetAddress, event);
    let calculatedAmount = rayDiv(userBalanceChange, index);
...
export function handleBalanceTransfer(event: BalanceTransfer): void {
  ...
  // TODO: is this really necessary(from v1)? if we transfer bToken we are not moving the collateral (underlying token)
  let bToken = getOrInitSubToken(event.address);
  let userFromReserve = getOrInitUserReserve(
    event.params.from,
    bToken.underlyingAssetAddress,
    event
  );
  let userToReserve = getOrInitUserReserve(event.params.to, bToken.underlyingAssetAddress, event);

  let reserve = getOrInitReserve(bToken.underlyingAssetAddress, event);
  if (
...
export function handleStableTokenBurn(event: STokenBurn): void {
  ...
  poolReserve.totalBTokenSupply = poolReserve.totalBTokenSupply.plus(balanceIncrease);
...
// export function handleBTokenBurn(event: Burn): void {
//   log.error('Burn ---------------------------------', []);
//   // tokenBurn(event, event.params.from, event.params.value, event.params.index);
// }

// export function handleBTokenMint(event: Mint): void {
//   log.error('Mint ---------------------------------', []);
